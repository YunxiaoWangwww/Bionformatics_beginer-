...
function Map:init()
    ...
    while x < self.mapWidth do
        ...
        -- make mario ladder blocks 
        elseif x > 10 and math.random(30) == 1 then  
            for y = 1, 4 do
                for z = 0, 4-y do 
                    self:setTile(x+y-1 +z, self.mapHeight/2-y,TILE_BRICK)
                end
            end
            for y = self.mapHeight/2, self.mapHeight do
                self:setTile(x,y,TILE_BRICK)
                self:setTile(x+1,y,TILE_BRICK)
                self:setTile(x+2,y,TILE_BRICK)
                self:setTile(x+3,y,TILE_BRICK)
            end
        ...
        elseif math.random(10) ~= 1 then
            ...
            -- next vertical scan line
            x = x + 1
        elseif  x < self.mapWidth -17 then
    ...
    self:setTile(self.mapWidth -1, self.mapHeight / 2 -4, FLAG_FLAG)
    for y = self.mapHeight/2, self.mapHeight do
        self:setTile(x,y,TILE_BRICK)
        self:setTile(x+1,y,TILE_BRICK)
        self:setTile(x+2,y,TILE_BRICK)
